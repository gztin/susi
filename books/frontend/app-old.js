    ...
    ? `<div class="price-block">
        ...
        ${Object.entries(card.tcgplayer.prices).map(([type, p]) => `
          <div class="price-row">
            ...
            <span class="price-item low">最低 $${p.low?.toFixed(2) ?? '—'} USD</span>
            <span class="price-item mid">中間 $${p.mid?.toFixed(2) ?? '—'} USD</span>
            <span class="price-item market">市場 $${p.market?.toFixed(2) ?? '—'} USD</span>
          </div>`).join('')}
