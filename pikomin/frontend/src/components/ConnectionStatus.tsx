...
export function ConnectionStatus({ websocketState }: ConnectionStatusProps) {
  ...
  const frontendPort = endpointPort(status.endpoints.frontend)
  const apiPort = endpointPort(status.endpoints.api)
      ...
      {isOpen && (
        <div className="connection-status-panel">
          <div className="connection-status-heading">
            <div>
              <span>運作狀態</span>
              <strong>{status.endpoints.integrated ? 'Pikmin Lite 整合服務' : '前後台連線'}</strong>
            </div>
          ...
          <dl className="connection-status-list">
            <div>
              ...
              <dd><code>{frontendPort}</code><span>此頁運作中</span></dd>
            </div>
            <div>
              <dt><StatusDot state={status.apiOnline ? 'online' : status.isLoading ? 'pending' : 'offline'} />後台 API</dt>
              <dd><code>{apiPort}</code><span>{status.apiOnline ? '已連線' : status.isLoading ? '確認中' : '無回應'}</span></dd>
            </div>
            <div>
